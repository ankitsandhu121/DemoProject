...
5. Prefer `page.getByTestId()` from the planner's candidate map over role, text, or CSS locators.
...
7. When an element has no `data-testid` in the dev source (`apps/web/src/`), do not add one and do not edit app code: discover the locator from the live DOM with the Playwright CLI, follow the `source-grounded-locators` priority order, and leave a TODO for the missing test id. **Never edit `apps/web/**` or `apps/api/**` to create a locator.**
