...
8. When an element needs a locator and has no `data-testid` in the dev source (`apps/web/src/`), build a **first-class fallback** per `source-grounded-locators` (step 3) — prefer a stable `data-testid` anchor with a scoped role/text child, use a regex accessible-name for dynamic/runtime labels — discovered from the live DOM with the Playwright CLI, leaving a TODO for the missing `data-testid`. A good fallback is durable coverage, not a workaround. **Never edit `apps/web/**` or `apps/api/**` to add a `data-testid` or create a locator.**
